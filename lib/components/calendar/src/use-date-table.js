...
const useDateTable = (props, emit) => {
  ...
  const rows = vue.computed(() => {
    ...
    if (isInRange.value) {
      ...
      const currentMonthRange = utils.rangeArr(
      ...
      const nextMonthRange = utils.rangeArr(remaining).map(
        (_, index) => ({
          text: index + 1,
          type: "next"
        })
      ...
      const prevMonthDays = dateTable.getPrevMonthLastDays(
        props.date,
      ...
      const currentMonthDays = dateTable.getMonthDays(props.date).map(
        (day) => ({
          text: day,
          type: "current"
        })
      ...
      const nextMonthDays = utils.rangeArr(remaining).map(
        (_, index) => ({
          text: index + 1,
          type: "next"
        })
